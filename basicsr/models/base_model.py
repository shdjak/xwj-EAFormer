...
class BaseModel():
    ...
    def load_network(self, net, load_path, strict=True, param_key='params'):
        ...
        # 筛除不加载的层结构:如果键名 k 同时存在于 load_net 和 model_dict 中，那么这个键值对(k, v)就会被包含在新创建的 pretrained_dict 字典中。
        # 最终，pretrained_dict 只包含那些可以被加载到模型中的参数，排除了那些在模型中不存在的参数。
        pretrained_dict = {k: v for k, v in load_net.items() if k in model_dict}

        self._print_different_keys_loading(net, pretrained_dict, strict)
        # 更新当前网络的结构字典
        model_dict.update(pretrained_dict)
        net.load_state_dict(model_dict, strict=strict)
